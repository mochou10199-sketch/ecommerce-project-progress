...
test.skip("legacy cloud: provides a non-destructive production acceptance smoke script", async () => {
...
test.skip("legacy cloud: gates production D1 export behind explicit credentials and validates core tables", async () => {
...
test.skip("legacy cloud: provides a read-only production role schema gate before deployment", async () => {
...
test.skip("legacy cloud: verifies the complete local D1 migration chain before release", async () => {
...
test.skip("legacy cloud: provides a repeatable isolated local acceptance flow", async () => {
...
test("uses local-only GitHub distribution and Node SQLite by default", async () => {
  const [db, localDb, readme, environment, packageJson] = await Promise.all([
    readFile(new URL("../db/index.ts", import.meta.url), "utf8"),
    readFile(new URL("../db/local-sqlite.ts", import.meta.url), "utf8"),
    readFile(new URL("../README.md", import.meta.url), "utf8"),
    readFile(new URL("../.env.example", import.meta.url), "utf8"),
    readFile(new URL("../package.json", import.meta.url), "utf8"),
  ]);
  assert.match(db, /getLocalDb/);
  assert.match(localDb, /DatabaseSync/);
  assert.match(localDb, /LOCAL_DB_PATH/);
  assert.match(readme, /项目源码只发布到 GitHub/);
  assert.match(readme, /不依赖 Vercel、Supabase、Cloudflare/);
  assert.match(environment, /LOCAL_DB_PATH/);
  assert.doesNotMatch(packageJson, /DATABASE_URL/);
  assert.doesNotMatch(packageJson, /build:vercel/);
});

test("provides local document upload, parsing and indexing", async () => {
  const [route, component, schema, query, acceptance] = await Promise.all([
    readFile(new URL("../app/api/projects/[id]/documents/route.ts", import.meta.url), "utf8"),
    readFile(new URL("../app/components/ProjectDocuments.tsx", import.meta.url), "utf8"),
    readFile(new URL("../db/sqlite-schema.ts", import.meta.url), "utf8"),
    readFile(new URL("../app/lib/project-query.ts", import.meta.url), "utf8"),
    readFile(new URL("../scripts/local-sqlite-acceptance.mjs", import.meta.url), "utf8"),
  ]);
  assert.match(route, /request\.formData\(\)/);
  assert.match(route, /MAX_FILE_BYTES/);
  assert.match(route, /TextDecoder/);
  assert.match(route, /documentChunks/);
  assert.match(component, /上传并建立索引/);
  assert.match(schema, /project_documents/);
  assert.match(schema, /document_chunks/);
  assert.match(query, /searchDocumentRows/);
  assert.match(acceptance, /indexedHits/);
});
